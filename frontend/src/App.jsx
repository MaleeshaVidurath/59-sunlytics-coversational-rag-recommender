...
const C = {
  ...
  card:      "#1c1c1c",
  border:    "#2a2a2a",
  accent:    "#c9a96e",      // warm gold — fashion brand feel
  accentDim: "#8a6f3e",
  user:      "#1e3a2f",      // deep forest green for user bubbles
  bot:       "#1c1c1c",
  text:      "#f0ebe3",
  textDim:   "#8a8078",
  textMuted: "#555",
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 10, color: C.textMuted, fontFamily: "monospace",
        flexShrink: 0 }}>#{item.article_id?.slice(-6)}</div>
    </div>
  );
}

function MetaBadge({ label, confidence, action, hallucination, contradiction }) {
  return (
    <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginTop: 8 }}>
      {label && (
        <span style={{
          background: C.tag, border: `1px solid ${labelColor(label)}33`,
          color: labelColor(label), fontSize: 10, padding: "2px 7px",
          borderRadius: 12, fontFamily: "monospace", letterSpacing: 0.3,
        }}>
          {label}
        </span>
      )}
      {confidence > 0 && (
        <span style={{
          background: C.tag, border: `1px solid #333`,
      ...
      {hallucination && (
        <span style={{
          ...
          fontWeight: 600,
        }}>⚠ hallucination flagged</span>
      )}
      {contradiction && (
        <span style={{
          background: C.contra, color: C.contraText,
          fontSize: 10, padding: "2px 8px", borderRadius: 12,
          fontWeight: 600,
        }}>✓ contradiction corrected</span>
      )}
    </div>
  );
}
    ...
    <div style={{
      ...
      {isUser && (
        <div style={{
          ...
          display: "flex", alignItems: "center", justifyContent: "center",
          fontSize: 13, marginLeft: 10, marginTop: 2, color: C.accent,
        }}>U</div>
      )}
    </div>
  );
...
function TypingIndicator() {
  return (
    <div style={{ display: "flex", padding: "0 16px", marginBottom: 16, alignItems: "center" }}>
      ...
      <div style={{
        background: C.bot, border: `1px solid ${C.border}`,
...
function SidebarItem({ session, active, onClick }) {
  return (
...
function LoginPage({ onLogin }) {
  ...
  const handleLogin = async () => {
    ...
    setError("");
    try {
      const user = await api.login(selected);
      onLogin(user);
    } catch {
  ...
  return (
    <div style={{
      ...
      <div style={{
        width: 440, background: C.sidebar,
        border: `1px solid ${C.border}`, borderRadius: 16,
        padding: "48px 40px", textAlign: "center",
      }}>
        {/* Logo */}
        <div style={{
          fontSize: 36, letterSpacing: 6, color: C.accent,
          fontWeight: 700, marginBottom: 8,
        }}>SUNLYTICS</div>
        <div style={{
          fontSize: 12, letterSpacing: 3, color: C.textDim,
          textTransform: "uppercase", marginBottom: 40,
          fontFamily: "system-ui, sans-serif",
        }}>Fashion Intelligence System</div>

        <div style={{
          color: C.textDim, fontSize: 13, marginBottom: 20,
          fontFamily: "system-ui, sans-serif",
        }}>
        ...
        {loading ? (
          <div style={{ color: C.textMuted, fontSize: 13 }}>Loading customers...</div>
        ) : (
          <>
            <select
              value={selected}
            ...
            {selected && (
              <div style={{
                ...
                textAlign: "left", fontSize: 12, fontFamily: "system-ui, sans-serif",
              }}>
                {customers.find(c => c.customer_id === selected) && (() => {
                  const c = customers.find(x => x.customer_id === selected);
                  return (
                    <>
                      <div style={{ color: C.textDim }}>
                        ID: <span style={{ color: C.accent, fontFamily: "monospace" }}>
                          {c.customer_id.slice(0,24)}...
                        </span>
                      </div>
                      {c.age && <div style={{ color: C.textDim }}>Age: {c.age}</div>}
                      {c.club_member_status && <div style={{ color: C.textDim }}>
                        Status: {c.club_member_status}
                      </div>}
                      {c.fashion_news_frequency && <div style={{ color: C.textDim }}>
            ...
            {error && (
              <div style={{
                color: C.flagText, background: C.flag,
            ...
            <button
              ...
              style={{
                width: "100%", background: selected ? C.accent : C.textMuted,
...
function ChatPage({ user, onLogout }) {
  ...
  const selectSession = async (session) => {
    ...
    try {
      const data = await api.getSessionHistory(session.session_id, user.user_id);
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            letterSpacing: 3, fontFamily: "'Playfair Display', Georgia, serif",
            marginBottom: 12,
          }}>SUNLYTICS</div>
          ...
          <button onClick={newChat} style={{
            ...
            border: `1px solid ${C.border}`, borderRadius: 8,
            color: C.textDim, padding: "8px 12px",
        ...
        <div style={{
          ...
          {sessions.length === 0 ? (
            <div style={{ color: C.textMuted, fontSize: 12, padding: "16px 4px" }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              width: 32, height: 32, borderRadius: "50%",
              background: C.user, border: "1px solid #2d5a3d",
              display: "flex", alignItems: "center", justifyContent: "center",
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ color: C.textDim, fontSize: 11 }}>
                {user.purchase_summary?.dominant_colour || ""} · {user.purchase_summary?.budget_tier || ""}
              </div>
            </div>
        ...
        <div style={{
          ...
          {activeSession && (
            <div style={{
              ...
              {user.purchase_summary?.total_purchases || 0} past purchases
            </div>
          )}
        </div>
        ...
        <div style={{
          ...
          {messages.length === 0 && !sending ? (
            <div style={{
              ...
              <div style={{
                fontSize: 42, letterSpacing: 6, color: C.accentDim,
                fontFamily: "'Playfair Display', Georgia, serif",
                marginBottom: 12,
              }}>SUNLYTICS</div>
              <div style={{ fontSize: 14, maxWidth: 340, textAlign: "center", lineHeight: 1.7 }}>
                Your personalised fashion assistant.<br />
                Tell me what you're looking for today.
              </div>
              <div style={{
        ...
        <div style={{
          ...
          >
            ...
            <button
              ...
              style={{
                background: input.trim() && !sending ? C.accent : C.textMuted,
...
export default function App() {
  ...
  const handleLogin = (userData) => {
    localStorage.setItem("sunlytics_user", JSON.stringify(userData));
    setUser(userData);
  };

  const handleLogout = () => {
    localStorage.removeItem("sunlytics_user");
    setUser(null);
  };
